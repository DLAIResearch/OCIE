...
def get_model(arch='vgg16',
    ...
    Args:
        arch (str, optional): name of architecture. If :attr:`dataset`
            contains ``"imagenet2"``, all :mod:`torchvision.models`
        ...
        dataset (str, optional): name of dataset, should contain
            ``'imagenet2'``, ``'voc'``, or ``'coco'``. Default: ``'voc'``.
    ...
    if 'voc' in dataset:
        ...
        num_classes = 80
    elif 'imagenet2' in dataset:
        num_classes = 1000
    else:
        assert False, 'Unknown dataset {}'.format(dataset)

    # Get/load the model from torchvision.
    model = models.__dict__[arch](pretrained='imagenet2' in dataset)

    if arch == 'inception_v3':
        model.aux_logits = False

    if 'imagenet2' not in dataset:
...
def get_transform(dataset='imagenet2', size=224):
    r"""
    Returns a composition of standard pre-processing transformations for
    feeding models. For non-ImageNet dataset, the transforms are
    for models converted from Caffe (i.e., Caffe pre-processing).

    Args:
        dataset (str): name of dataset, should contain either ``'imagenet2'``,
            ``'coco'`` or ``'voc'`` (default: ``'imagenet2'``).
