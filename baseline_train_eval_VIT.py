...
parser.add_argument('-a', '--arch', metavar='ARCH', default='DeiT',
                    choices=model_names,
...
parser.add_argument('--dataset', type=str, default='HAM',
                            help='dataset to use: [imagenet, cub, HAM, cars]')
...
def validate(val_loader, model, criterion, args, logger):
    ...
    top1 = AverageMeter('Acc@1', ':6.2f')
    top5 = AverageMeter('Acc@5', ':6.2f')
    recalls = AverageMeter('recall', ':.6e')
    f1s = AverageMeter('f1', ':.6e')
    precisions = AverageMeter('precision', ':.6e')
    progress = ProgressMeter(
        len(val_loader),
    ...
    with torch.no_grad():
        ...
        for i, (images, target) in enumerate(val_loader):
            ...
            recall = torch.tensor(recall_score(target.cpu(), class_output, average='weighted'),
                                  dtype=torch.float32).cuda()
            f1 = torch.tensor(f1_score(target.cpu(), class_output, average='weighted'),
                              dtype=torch.float32).cuda()  # measure accuracy and record loss
            precision = torch.tensor(precision_score(target.cpu(), class_output, average='weighted'),
                                     dtype=torch.float32).cuda()
            losses.update(loss.item(), images.size(0))
            top1.update(acc1[0], images.size(0))
            top5.update(acc5[0], images.size(0))
...
def save_checkpoint(state, is_best, save_dir):
    ...
    if is_best:
        best_filename = 'DeiT_base_model_best_HAM.pth.tar'
