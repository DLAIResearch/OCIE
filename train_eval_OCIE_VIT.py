...
parser.add_argument('--log_dir', default='logs-imagenet-ViT', type=str, metavar='LG_PATH',
                    help='path to write logs (default: logs)')
parser.add_argument('--dataset', type=str, default='imagenet',
                            help='dataset to use: [imagenet,cars,cub]')
...
def train(train_loader, model, engry_criterion, xent_criterion,align_loss, optimizer, epoch, args, logger):
    ...
    for i in range(train_len):
        ...
        # class_indices = torch.argmax(targets, axis=1)
        pred=pred.cuda(args.gpu, non_blocking=True)
        aug_images_outputs=model(aug_images)
        images_outputs=model(images)
        xe_images_output=model(xe_images)
        xe_loss = xent_criterion(images_outputs, targets.cuda()) + xent_criterion(aug_images_outputs,
                                                                        targets.cuda()) + xent_criterion(xe_images_output, targets.cuda())
        attribution_generator = LRP(model)
        orig_gradcam_mask = generate_visualization(attribution_generator,images , targets).cuda()
        aug_gradcam_mask = generate_visualization(attribution_generator, aug_images, targets).cuda()

        engry_criterion_loss2 = engry_criterion(pred.float(), aug_gradcam_mask)
        align_loss2 = align_loss(aug_gradcam_mask.flatten(1), (orig_gradcam_mask * pred.float()).flatten(1))
        xe_loss = xe_loss.mean()
        engry_criterion_loss2 = engry_criterion_loss2.mean()
        align_loss2=align_loss2.mean()
        loss = xe_loss + args.lambda_val * engry_criterion_loss2+args.beta_val*align_loss2

        # measure accuracy and record loss
        acc1, acc5 = accuracy(aug_images_outputs, targets, topk=(1, 5))
